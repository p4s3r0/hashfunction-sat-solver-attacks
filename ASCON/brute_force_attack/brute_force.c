...
int main(int argc, char *argv[])
{
	...
	INPUT_LENGTH = 64 / 8;
	ASCON_HASH_ROUNDS = strtoll(argv[1], NULL, 10);
	CRYPTO_BYTES   = strtoll(argv[2], NULL, 10) / 8;
	int HashIndex = strtoll(argv[3], NULL, 10);


	/*message = generate_mystery_value();
	message = malloc(INPUT_LENGTH);
	for(int i = 0; i < INPUT_LENGTH; i++)
		message[i] = 0xff;

	unsigned char digest[CRYPTO_BYTES];
	crypto_hash(digest, message, INPUT_LENGTH);

	hash_to_check = digest;
	*/

	hash_to_check = malloc(INPUT_LENGTH * sizeof(char));
	/*for(int i = 0; i < INPUT_LENGTH; i++)
		hash_to_check[i] = 0xff;*/
	...
	printf("%08llx\n0x", all_hashes[HashIndex]);

	for(int i = 0; i < INPUT_LENGTH/2; i++)
	{
		hash_to_check[i] = all_hashes[HashIndex] >> ((3 - i) * 8) & 0xff;
...
/*
	...
	fprintf(fptr, "\n\tprec: 0x");
	for (unsigned long long i = 0; i < CRYPTO_BYTES; i++)
		fprintf(fptr, "%02X", hash_to_check[i]);
	fprintf(fptr, "\n\tPimg: 0x");
	for (unsigned long long i = 0; i < CRYPTO_BYTES; i++)
		fprintf(fptr, "%02X", found_pre_image[i]);
	fprintf(fptr, "\n\ttime: %.3fs\n}\n", elapsed_time);
	fclose(fptr);*/
}
